...
对于蛋白质嵌入，我们限制了kNN搜索，只考虑属于同一基因组簇内的蛋白质。蛋白质嵌入进行了单位规范化，使得每个蛋白质嵌入的L2范数等于1。然后，我们使用余弦相似度来选择kNN集合，余弦相似度得分被用作kNN图的边权重。对于蛋白质少于78个的基因组簇，对所有蛋白质对进行了暴力计算余弦相似度。对于较大的基因组簇，使用了IndexIVFFlat分区方法，其中 )*		+ 为m，因为39是每个Voronoi细胞的最小数据点数。然后，使用Leiden算法的Python实现（iGraph v0.11.3）在相似性加权的kNN图中检测基因组或蛋白质簇。我们使用的分辨率值为0.1（“中”）和1.0（“高”）用于基因组聚类，以及0.1（“低”）和0.5（“中”）用于蛋白质聚类。我们在下游分析中不包括单体作为簇。  
